...
case 'initialize';
  ...
  for i=1:2;
    S_HMenuParamList(i)=uimenu(S_HMenuParam,'label','', ...
      'enable','off', ...
      'callback',['st_param(''menu'',' num2str(i) ')']);
  end;
  for f1=get(0,'children')';
    if strcmp(get(f1,'name'),'STEPTEST PARAM EDIT');
      delete(f1);
    end;
  end;
   S_ControlParam.MinStepTime=NaN;
   S_ControlParam.LatencyTime=NaN;
case 'menu';
  paramval=[S_ControlParam.MinStepTime S_ControlParam.LatencyTime];
  for f1=get(0,'children')';
    if strcmp(get(f1,'name'),'STEPTEST PARAM EDIT');
      delete(f1);
    end;
  ...
  set(S_ParamFig,'position',[150   300   300 50],'numbertitle','off', ...
      'name','STEPTEST PARAM EDIT','menubar','none');
  ...
  S_HParamEdit=uicontrol(S_ParamFig,'style','edit',...
        ...
        'callback',['st_param(''edit'',' num2str(act2) ')']);
case 'edit';
  switch act2;
  case 1;
    S_ControlParam.MinStepTime=round(str2num(get(S_HParamEdit,'string'))/2)*2
...
case 'update';
  set(S_HMenuParamList(1),'label', ...
          sprintf('Ramp Time:    %6d ms',S_ControlParam.MinStepTime));
