...
M=[4 2 4];   %Number of curves per trace
if ToolStruct.Init;
    ToolStruct.Init=0;
    N=1;
    orient tall;
    set(ToolStruct.Figure,'position',[1280         202         634         908]);
    itr=1;
        subplot(NTR,1,itr);
...
for iM=1:2
  set(ToolStruct.TR(itr).line(iM),'xdata',ALGORAV.lCurrentStepAv(1:N)', ...
    'ydata',ALGORAV.TempAv(1:N,iM+2)); 
  set(ToolStruct.TR(itr).text(iM),'string',sprintf('C%d=% 11.4g' ...
    ,iM+2,ALGORAV.TempAv(N,iM+2)));
end
% for iCon=1:1
%     iM=S_Mast.NCon + iCon;
%   set(ToolStruct.TR(itr).line(iM),'xdata',ALGORAV.lCurrentStepAv(1:N)', ...
