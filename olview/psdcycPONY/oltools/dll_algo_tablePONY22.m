...
s=[s sprintf('\n')];
% s=[s sprintf(' iRec:%5g   RunAlgo-F11:%4g   TriggerIn-CH3:%4g', ...
%     ALGOIREC(1).iRec(N),PSD(1).bRunAlgo(N),DEV_IN(1).CHANNEL_3(N))];
s=[s sprintf(' iRec:%5g   RunAlgo-F11:%4g    MstDigIn3:%4g', ...
    ALGOIREC.iRec(N),PSD.bRunAlgo,MST_DI1_IN.MstDigIn3)];
%     ALGOIREC.iRec(N),PSD.bRunAlgo(N),MST_DI1_IN.MstDigIn3(N))];   %Marco
s=[s sprintf('\n')];
...
%     ALGORUSERINPUT.InterRecIn(N),ALGOR_T.TimeIncr(N),ALGORUSERINPUT.TimeLambda(N))];%Marco
%     ALGORUSERINPUT.InterRecIn(N),ALGOR_T.TimeIncr(N),ALGORUSERINPUT.TimeLambda(N))];

s=[s sprintf('\n+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')];
s=[s sprintf('\nCount-t:%4g                            TimeStop:%6.4f  Time-t:%6.4f ', ...
    ALGOR_T.Count_t(N),ALGORUSERINPUT.TimeStop,ALGOR_T.Time_t(N))];
%     ALGOR_T.Count_t(N),ALGORUSERINPUT.TimeStop(N),ALGOR_T.Time_t(N))];%Marco
...
s=[s sprintf('\n Target-t [mm]:   ') sprintf('% 13.4e',ALGOR_T.DisConTarget_t(N,:))];
