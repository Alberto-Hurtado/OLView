function [polesHz,modes,poles]=timepole_new(x,u,time_increment,order)
...
%u=rand(nt,1);
%x=sdofresp(u,dt,f,z);
%[polesHz]=timepole_new(x,u,dt,order)
%
%f1=5; f2=8; z=0.08; dt=0.01; order=2; nt=10;
%u=rand(nt,1);
%x=sdofresp(u,dt,[f1 f2],z);
%[polesHz,modes]=timepole_new(x,u,dt,order)
%
%f1=5; f2=8; z1=0.05; z2=0.08; dt=0.01; order=2; nt=7;ndof=2;
%t=dt*[1:nt]';
...
for imode=1:size(modes,2)   % 2008 01 25
    [dum,jmax]=max(modes([1:ndof],imode));
    modes(:,imode)=modes(:,imode)./modes(jmax,imode);
end


%Scalar product with reference mode shape
% refshape=ones(size(modes,1),1);


% refshape=zeros(size(modes,1),1);
% refshape(1:4)=[0.5 0.5 1 1]';
% 
% prod=1./(refshape'*modes);
% 
% freq_param=1; damp_param=20; shape_param=200;
% 
% price=freq_param*polesHz(:,1)+damp_param*polesHz(:,2).^2+shape_param*prod';
% 
% [pdum,ind]=sort(price);
% poles=poles(ind);
% polesHz=polesHz(ind,:);
