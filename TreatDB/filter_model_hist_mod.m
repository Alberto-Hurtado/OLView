function [freq,zeta,t,modes]=filter_model_hist_mod(d,u,Dt,order,Npwin,Npjump)
%filter_model_hist_mod:    Identification of frequency and damping
%                  history from dynamic output displacement and input history.
%                  This function is the same as filter_model_hist, but with the
%                  added output of the mode shapes.
%
%       [freq,zeta,t,modes]=filter_model_hist_mod(d,u,Dt,order,Npwin,Npjump)
%
...
%   modes(NDoF,Nmodes,Np_id,,Norders)        Identified mode shapes (Nmodes=NDoF).
%
%
...
modes=zeros(NDoF,NDoF,Np_id,Norders); 
for iwin=1:Np_id;
    ...
    for i1=1:Norders;
        [polesHz,mod]=filter_model_poles(dd,uu,Dt,order(i1));
        freq(iwin,:,i1)=polesHz(1:NDoF,1)';
        zeta(iwin,:,i1)=100*polesHz(1:NDoF,2)';
        modes(:,:,iwin,i1)=mod(1:NDoF,1:NDoF);
        for imode=1:NDoF  % Normalises the maximum component of every mode to 1
            modes(:,imode,iwin,i1)=modes(:,imode,iwin,i1)./max(modes(:,imode,iwin,i1));
        end
    end
end
